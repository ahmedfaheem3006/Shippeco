        ...
        {totalInvoices > 0 && (
          <div className="p-4 border-t border-gray-100 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50/50 dark:bg-slate-800/30">
            <div className="text-xs font-bold text-gray-500 dark:text-slate-400">
              ...
              <span className="text-gray-900 dark:text-white font-mono">{Math.max(1, Math.ceil(totalInvoices / 20))}</span> (إجمالي {totalInvoices} فاتورة)
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1">
                {Array.from({ length: Math.min(5, Math.ceil(totalInvoices / 20)) }, (_, i) => {
                  const totalPages = Math.ceil(totalInvoices / 20);
                  let pNum = page;
                  if (totalPages <= 5) {
                    pNum = i + 1;
                  } else if (page <= 3) {
              ...
              <button
                onClick={() => setPage((p) => Math.min(Math.ceil(totalInvoices / 20), p + 1))}
