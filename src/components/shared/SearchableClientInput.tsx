...
type Props = {
  nameValue: string
  phoneValue: string
  onSelect: (name: string, phone: string, email?: string, category?: string) => void
...
export function SearchableClientInput({ nameValue, phoneValue, onSelect, onNameChange, onPhoneChange }: Props) {
  ...
  const handleSelect = (name: string, phone: string, email?: string | null, category?: string | null) => {
    onSelect(name, phone, email || undefined, category || undefined)
    setHasSelected(true)
    setOpen(false)
    setResults([])
  }
            ...
            <div className="p-1.5">
              ...
              {results.map((r, idx) => (
                <button
                  ...
                  onClick={() => handleSelect(r.name, r.phone, r.email, r.collection_category)}
                >
